...
describe('UsersService', () => {
  ...
  describe('create', () => {
    it('should create a new user and emit an event', async () => {
      const createUserDto = {
        name: 'rizwan',
  ...
  describe('axios.get', () => {
    it('should return user data', async () => {
      ...
      // Mock the axios.get method to resolve with the userData
      jest
        .spyOn(axios, 'get')
        .mockResolvedValueOnce({ data: { data: userData } });

      // Call the axios.get method directly
      const response = await axios.get(`https://reqres.in/api/users/${userId}`);

      // Extract the data
      const result = response.data.data;

      // Check if the result is equal to userData
  ...
  it('should return avatar data if user has avatar', async () => {
    const userId = '1';
    ...
    jest.spyOn(userModel, 'findOne').mockReturnValueOnce({
      exec: jest.fn().mockResolvedValueOnce(userWithAvatar),
    } as any);

    const avatarData = await usersService.getAvatar(userId);

    expect(avatarData).toEqual(userWithAvatar.avatar);
  });

  // it('should fetch avatar from external API and save to file system if user does not have avatar', async () => {
  //   const userId = '1';
  ...
  //   jest.spyOn(userModel, 'findOne').mockReturnValueOnce({
  //     exec: jest.fn().mockResolvedValueOnce(userWithoutAvatar),
  //   } as any);

  //   jest
  //     .spyOn(usersService, 'findById')
  //     .mockResolvedValueOnce(response.data.data);
  ...
  it('should delete avatar from file system and update user record', async () => {
    const userId = '1';
    const user = {
      userId: userId,
      avatar:
    ...
    const deleteAvatarSpy = jest
      .spyOn(usersService, 'deleteAvatar')
      .mockImplementationOnce(async (userId: string) => Promise.resolve());
